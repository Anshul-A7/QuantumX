...
export default function NeurologicalDetailPage() {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="rounded-3xl border border-amber-500/30 bg-card p-8 sm:p-10 shadow-xl space-y-6 text-center relative overflow-hidden">
        ...
        <div className="space-y-2 max-w-lg mx-auto">
          ...
          <h1 className="font-serif text-3xl font-light text-foreground tracking-tight">
            Neurological & Brain Health Studio
          </h1>
          <p className="text-xs text-muted-foreground leading-relaxed pt-1">
            The Neurological screening pipeline is currently undergoing multi-channel EEG spectral calibration and quantum Hamiltonian entanglement cross-validation.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-xl mx-auto text-left pt-2">
          ...
          <div className="p-3.5 rounded-xl border border-border bg-muted/20 space-y-1">
            ...
            <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Heart Attack (ECG)</p>
            <p className="text-[11px] text-muted-foreground">Dual-Engine CX-01 & Transfinite-1.</p>
          </div>
          <div className="p-3.5 rounded-xl border border-border bg-muted/20 space-y-1">
            <span className="text-[10px] font-mono uppercase text-muted-foreground font-semibold">Verification</span>
            <p className="text-xs font-semibold text-foreground">EEG Wavelet VQC</p>
        ...
        <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
          ...
          >
            <Sparkles size={14} />
            <span>Launch Heart Attack Cardiac Studio</span>
            <ArrowRight size={14} />
          </Link>
          <Link
            href="/predict"
